...
class BaseConfluencePage(ConfluenceConnector):
    ...
    def delete_page(self):
        ...
        if page_id:
            r = requests.delete('{}/{}'.format(self.api_url, page_id), auth=(self.username, self.password), verify=self.verify_requests)
    ...
    def cache_page(self):
        ...
        while not successful:
            try:
                r = requests.get(self.api_url, auth=(self.username, self.password), params=params, verify=self.verify_requests)
                successful = True
            except:
    ...
    def get_labels(self):
        if self.page_exists():
            page_id = self.get_page_id()
            r = requests.get(self.api_url + "/" + page_id + "/label", auth=(self.username, self.password), verify=self.verify_requests)
    ...
    def attachment_exists(self, filename):
        if self.page_exists():
            ...
            r = requests.get(self.api_url + "/" + page_id + "/child/attachment", auth=(self.username, self.password), params=params, verify=self.verify_requests)
    ...
    def create_page(self, page_text):
        ...
        if not self.page_exists():
            ...
            r = requests.post(self.api_url, auth=(self.username, self.password), data=json.dumps(data), headers=({'Content-Type':'application/json'}), verify=self.verify_requests)
    ...
    def commit_page(self):
        ...
        try:
            ...
            r = requests.put(self.api_url + "/" + page_id, auth=(self.username, self.password), data=json.dumps(data), headers=({'Content-Type':'application/json'}), verify=self.verify_requests)
        except:
    ...
    def attach_file(self, file_path):
        if self.page_exists():
            ...
            if os.path.exists(file_path):
                ...
                r = requests.post(self.api_url + "/" + page_id + "/child/attachment", auth=(self.username, self.password), files=attachment, headers=({'X-Atlassian-Token':'nocheck'}), verify=self.verify_requests)
    ...
    def add_page_label(self, label):
        if self.page_exists():
            ...
            r = requests.post(self.api_url + "/" + page_id + "/label", auth=(self.username, self.password), data=json.dumps(data), headers=({'Content-Type':'application/json'}), verify=self.verify_requests)
