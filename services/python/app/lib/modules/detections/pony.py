        ...
        for url in set([i['value'] for i in self.event_json['indicators'] if i['type'] == 'URI - URL']):
            ...
            if favicon_url:
                try:
                    ...
                    command = '{} wget -O {} -U {} -T {} -t {} {}'.format(PROXYCHAINS, temp.name, shlex.quote(user_agent), 5, 1, shlex.quote(favicon_url))
