...
class Module(DetectionModule):
    ...
    def run(self):
        ...
        ignore_domains = self.config['ignore_domains']
        ...
        for domain in unique_domains:
            try:
                ...
                command = '{} -q dig +noall +answer {} txt'.format(PROXYCHAINS, domain)
        ...
        for domain in unique_domains:
            try:
                command = '{} -q dig +noall +answer {} MX'.format(PROXYCHAINS, domain)
                output = subprocess.check_output(command, shell=True).decode('utf-8')
